...
| `aria-describedby` | `string` | — | Points to a description element. Wiring `<NumberField.Description>` (or any element spreading `descriptionProps`) is now automatic while it is mounted; a value you pass here is merged with it rather than required — see [Accessibility](/docs/guides/accessibility). |
